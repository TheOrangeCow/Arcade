...
def on_closing(pong):
    if pong.score1 > pong.score2:
        winner_score = pong.score1
        loser_score = pong.score2
    elif pong.score2 > pong.score1:
        winner_score = pong.score2
        loser_score = pong.score1
    else:
        winner_score = loser_score = pong.score1 

    score_difference = winner_score - loser_score
    add("Pong", score_difference)
    pong.destroy()

def ponggo():
    pong = Pong()
    pong.title("Pong")
    pong.protocol("WM_DELETE_WINDOW", lambda: on_closing(pong))
    pong.mainloop()
